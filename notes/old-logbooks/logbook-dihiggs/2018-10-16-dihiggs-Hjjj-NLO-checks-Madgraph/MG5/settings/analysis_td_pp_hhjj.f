...
c      
     ...
     &        nwgt_analysis*50*4
      ...
      do i=1,2
        l=(kk-1)*66+(i-1)*33 !replace 33, 66, if total of hists changes
     ...
     &       //cc(i)//weights_info(kk),5d-1,-6d0,6d0)
         call bookup(l+ 26,'min_yj1j3_yj3j2 '
     &       //cc(i)//weights_info(kk),2d-1, -5d0,5d0)
         call bookup(l+ 27,'rapj1j3 '
     &       //cc(i)//weights_info(kk),5d-1,0d0,10d0)
         call bookup(l+ 28,'rapj2j3 '
     &       //cc(i)//weights_info(kk),5d-1,0d0,10d0)
         
         call bookup(l+ 29,'ptj4 '
     &       //cc(i)//weights_info(kk),5d0,0d0,100d0)
         call bookup(l+ 30,'yj4 '
     &       //cc(i)//weights_info(kk),5d-1,-6d0,6d0)
         call bookup(l+ 31,'rapj1j4 '
     &       //cc(i)//weights_info(kk),5d-1,0d0,10d0)
         call bookup(l+ 32,'rapj2j4 '
     &       //cc(i)//weights_info(kk),5d-1,0d0,10d0)
         call bookup(l+ 33,'rapj3j4 '
     &       //cc(i)//weights_info(kk),5d-1,0d0,10d0)
      enddo
      enddo
      return
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do kk=1,nwgt_analysis
      do i=1,2
        l=(kk-1)*66+(i-1)*33 ! replace 33 if total of hists changes
         ...
         call multitop(l+ 26,3,2,'min_yj1j3_yj3j2  ',' ','LIN')
         call multitop(l+ 27,3,2,'rapj1j3          ',' ','LIN')
         call multitop(l+ 28,3,2,'rapj2j3          ',' ','LIN')

         call multitop(l+ 29,3,2,'ptj4             ',' ','LIN')
         call multitop(l+ 30,3,2,'yj4              ',' ','LIN')
         call multitop(l+ 31,3,2,'rapj1j4          ',' ','LIN')
         call multitop(l+ 32,3,2,'rapj2j4          ',' ','LIN')
         call multitop(l+ 33,3,2,'rapj3j4          ',' ','LIN')
      enddo
      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     $     ,pj1(0:3),pj2(0:3),pj3(0:3),pj4(0:3),pjj(0:3),pHH(0:3)
     $     ,ptH1,yH1,ptH2,yH2,ptHd,yHd,ptHs,yHs,ptHH,yHH,phiHH
     $     ,ptj1,yj1,ptj2,yj2,yj1j3,yj2j3,min_yj1yj2yj3
     $     ,Dphij1j2,mj1j2,Dyj1j2,ptj3,yj3,yj3rel,ptj4,yj4
     $     ,yj1j4,yj2j4,yj3j4
      logical pass_tag_cuts,flag
      integer nQCD,jet(nexternal),ij1y,ij2y,ij3y,ij4y,njet,njety
     $     ,ij1,ij2,ij3,ij4
     ...
     $     ,getinvmv4,getmod
      ...
      if (nexternal.ne.8) then
         write (*,*) 'error #1 in analysis_fill: '/
     &        /'only for process "p p > h h j j j [QCD]"'
         stop 1
      endif
     ...
     &        /'only for process "p p > h h j j j [QCD]"'
      ...
      if (.not. (abs(ipdg(7)).le.5 .or. ipdg(7).eq.21)) then
         write (*,*) 'error #6 in analysis_fill: '/
     &        /'only for process "p p > h h j j j [QCD]"'
         stop 1
      endif
      if (.not. (abs(ipdg(8)).le.5 .or. ipdg(8).eq.21)) then
         write (*,*) 'error #6 in analysis_fill: '/
     &        /'only for process "p p > h h j j j [QCD]"'
         stop 1
      endif
...
c At least three jets
      if(njet.ge.3)then
        ...
        min_yj1yj2yj3 = min(abs(yj1j3),abs(yj2j3))
...
c     We assign plus if the third jet is in the window and minus otherwise.
        min_yj1yj2yj3 = sign(min_yj1yj2yj3,-yj1j3*yj2j3) 
      endif
c At least four jets
      if(njet.ge.4)then
        ptj4    = getptv4(pj4)
        yj4     = getrapidityv4(pj4)
        yj1j4   = yj1 - yj4
        yj2j4   = yj2 - yj4
        yj3j4   = yj3 - yj4
      endif
c
     ...
     &                mj1j2.ge.mj1j2min 
      ...
      do kk=1,nwgt_analysis
      www=wgts(kk)
      l=(kk-1)*66+(i-1)*33 ! replace 66 and 33 if number of hists changes
      if(flag)then
         ...
         if(njet.ge.3)then
            ...
            call mfill(l+ 26,min_yj1yj2yj3,www)
            call mfill(l+ 27,abs(yj1j3),www)
            call mfill(l+ 28,abs(yj2j3),www)
         endif

         if(njet.ge.4)then
            call mfill(l+ 29,ptj4,  www)
            call mfill(l+ 30,yj4,   www)
            call mfill(l+ 31,abs(yj1j4),www)
            call mfill(l+ 32,abs(yj2j4),www)
            call mfill(l+ 33,abs(yj3j4),www)
         endif
      endif
      enddo
      enddo
