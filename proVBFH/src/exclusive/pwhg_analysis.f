...
c     do nothing, since we are running without tags and it's the qqqqqq channel
     ...
     $                 flst_alrtags(6,j) == 1 .and. flst_alrtags(7,j) == 2) then ! case 1 2 -> 1 2 1 2 
                  ig=3          ! 12 case
               elseif (sum(flst_alrtags(1:nlegreal,j)) == 9 .and. 
     $                 flst_alrtags(6,j) == 2 .and. flst_alrtags(7,j) == 1) then ! case 1 2 -> 1 2 2 1 
                  ig=4          ! 21 case
               else
...
c     Mapping for one or two gluons on upper line
      if (igmax .ge. 2) then 
         call mapping_1_1(psave(:,1),psave(:,4),psave(:,6),psave(:,7)
     $        ,p1,p2)
         psub(:,1,1)=p1
         psub(:,4,1)=p2
         psub(:,6,1)=0d0
...
c     Mapping for one or two  gluons on lower line
         call mapping_2_2(psave(:,2),psave(:,5),psave(:,6),psave(:,7)
     $        ,p1,p2)
         psub(:,2,2)=p1
         psub(:,5,2)=p2
         psub(:,6,2)=0d0
...
c     Mapping for Born gluon on upper line and real gluon on lower
         call mapping_1_2(psave(:,1),psave(:,4),psave(:,6)
     $        ,psave(:,2),psave(:,5),psave(:,7)
     $        ,p1,p2,p3,p4)
         psub(:,1,3)=p1
         psub(:,4,3)=p2
         psub(:,2,3)=p3
...
c     Mapping for Born gluon on lower line and real gluon on upper
         call mapping_1_2(psave(:,1),psave(:,4),psave(:,7)
     $        ,psave(:,2),psave(:,5),psave(:,6)
     $        ,p1,p2,p3,p4)
         psub(:,1,4)=p1
         psub(:,4,4)=p2
         psub(:,2,4)=p3
