...
\item\texttt{nf\_epsrel}: relative accuracy of the adaptive integration
  over the azimuth in the non-factorisable corrections (default
  $10^{-8}$).
  
\item\texttt{nf\_regfact}: gluon-mass regulator of the non-factorisable
  corrections, $\lambda = \texttt{nf\_regfact}\, M_V^2$ (default 1). The
  result does not depend on it; the default is the fastest choice.
