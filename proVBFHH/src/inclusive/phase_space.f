...
C     VBFNLO and matrix element computed with Hoppet
...
C     q1_perp, q1_perp_sq, Q2_sq, q2_perp using generated phase space
      call set_phsp()
! Lorenzo's kinematics
!      vq1(1) =  0.9000000000000000000000000d0
!      vq1(2) = 0d0
!      vq2(1) =  0.7000000000000000000000000d0 
!      vq2(2) = 3.100000000000000000000000d0
!      pH1(1)= 2.100000000000000000000000d0
!      pH1(2) = 0.5800000000000000000000000d0
!      pH2(:) = -vq1(:) - vq2(:) - pH1(:)
...
!         dsigma = eval_matrix_element_vbfnlo(order_min,order_max, x1, x2,
...
!         print*, 'vbfnlo', dsigma
     ...
     $           kn_beams(:,1), kn_beams(:,2), vq1, vq2, pH1, pH2, ptH1H2)
            ...
            if(tensorME) then
               dsigma = eval_matrix_element_tensor(order_min,order_max,
     $              x1, x2,kn_beams(:,1), kn_beams(:,2), vq1, vq2, pH1,
     $              pH2, ptH1H2)
!            print*, 'tensor', dsigma
            else
               dsigma = eval_matrix_element(order_min,order_max, x1, x2,
     $              kn_beams(:,1), kn_beams(:,2), vq1, vq2, pH1, pH2,
     $              ptH1H2)
            endif
