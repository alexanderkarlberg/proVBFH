...
!------------------------------------------------------------
!     
!     Main module for calculation of inclusive VBFHH 
!     
!------------------------------------------------------------
!------------------------------------------------------------
...
contains
  ...
  subroutine inclusive_init()
    ...
    call StartStrFct(sqrts, order_max, nflav, xmur, xmuf, scale_choice, mh, .true., Qmin, mw, mz)
    call read_PDF()
...
!    stop
    ...
    if (order_max.eq.1) then 
       ...
       write(11,'(es13.6,a,es13.6,a)') sigma_tot*1000d0, ' +/- ', sqrt(error_tot*1000000d0),' (fb)'
    else if (order_max.eq.2) then 
