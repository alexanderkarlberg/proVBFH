...
c initialize:
      ...
      ! VBFHH mod: bflav_tag and ftype indices
      ftype(8) =  2-mod(abs(bflav(8)),2)  !identify as up- or down-type

      do mu = 0,3
         
      ! outgoing (QQB) pair:
      ! VBFHH mod: bflav_tag indices
      if (bflav_tag(8).eq.(-bflav_tag(7))) then 
        col_del = 2


      ! VBFHH mod: bflav_tag indices
         if (bflav_tag(7).gt.0) then 
            qsign(1) = -1
            qsign(2) = +1
            !VBFHH mod: fixed pin indices
            qbar(mu,2) = pin(mu,7)
            qbar(mu,1) = pin(mu,8)
      ! VBFHH mod: bflav_tag indices
         elseif (bflav_tag(7).lt.0) then   
            qsign(1) = -1!+1
            qsign(2) = +1!-1
            !VBFHH mod: fixed pin indices
            qbar(mu,2) = pin(mu,8)
      ...
      ! VBFHH mod: bflav_tag indices
      elseif (bflav_tag(8).eq.(bflav_tag(1))) then 
            if (bflav_tag(1).ne.bflav_tag(2)) then
               pair81=.true.
            else   
      ! VBFHH mod: bflav_tag indices
               if (bflav_tag(5).lt.0) then
                  pair82=.true.
               else    
                  pair81=.true.
               endif   
      ...
      ! VBFHH mod: bflav_tag indices
      elseif (bflav_tag(8).eq.(bflav_tag(2))) then 
            pair82 = .true.
      endif    
 


      if (pair81) then !Q1 in, Q8 out
         if (bflav(8).lt.0) then 
            qsign(1) =-1
         qsign(2) =-1
      ! VBFHH mod: pin indices
         qbar(mu,2) = pin(mu,1)
         qbar(mu,1) = pin(mu,8)
      elseif (bflav(8).gt.0) then  
         qsign(1) =+1
         qsign(2) =+1
      ! VBFHH mod: pin indices
         qbar(mu,2) = pin(mu,8)
      ...
      elseif (pair82) then !Q1 in, Q8 out
      if (bflav(8).lt.0) then 
      ! VBFHH mod: pin indices
         qsign(1) =-1
         qsign(2) =-1
...
ccccccccccccccc
      ...
      elseif (pair81) then ! Q incoming, Q outgoing
      col_del = 3

      !VBFHH mod: ftype indices fixed
      if (bflav(2).gt.0.and.bflav(5).gt.0) then
...
c      if (nc_type) then !nc
      ...
      !VBFHH mod: bflav indices fixed   
      elseif (bflav(2).lt.0.and.bflav(5).gt.0) then
...
c up- or down-type quark:      
      ...
      !VBFHH mod: bflav indices fixed   
      elseif (bflav(1).lt.0.and.bflav(6).gt.0) then
...
!            res_pwg = res
     ...
     $           ,cc_type,nc_type)
            if(tag_type.ne.0) then
               if (kl.eq.1) then 
                  res_vbf = NCmatrix_r(0,0,0,tag_type,0)
               elseif (kl.eq.2) then         
                  res_vbf = NCmatrix_r(0,1,0,tag_type,0)
               elseif (kl.eq.3) then         
                  res_vbf = NCmatrix_r(1,0,0,tag_type,0)
               elseif (kl.eq.4) then         
                  res_vbf = NCmatrix_r(1,1,0,tag_type,0)
               elseif (kl.eq.5) then         
                  res_vbf = NCmatrix_r(0,0,1,tag_type,0)
               elseif (kl.eq.6) then         
                  res_vbf = NCmatrix_r(0,1,1,tag_type,0)
               elseif (kl.eq.7) then         
                  res_vbf = NCmatrix_r(1,0,1,tag_type,0)
               elseif (kl.eq.8) then         
                  res_vbf = NCmatrix_r(1,1,1,tag_type,0)               
               endif
            else
               if (kl.eq.1) then 
...
c k-ordering (5:6) is: udsc,ducs
 ...
 !    &         call qqHqqQQ_cc_channel(pbar_cc,fsign,kl,res)
     ...
     $           ,cc_type,nc_type)
            ...
            if(tag_type.ne.0) then
               if (k.eq.5) then  
                  res_vbf=CCmatrix_r(0,1,0,tag_type,0)
               elseif (k.eq.6)then
                  res_vbf=CCmatrix_r(1,0,0,tag_type,0)
               endif
!            print*, 'CCmatrix_r', CCmatrix_r
            else
               if (k.eq.5) then  
