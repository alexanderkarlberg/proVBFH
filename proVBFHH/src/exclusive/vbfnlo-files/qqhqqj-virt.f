...
c      real*8 uucc_c(3),uuss_c(3),ddcc_c(3),ddss_c(3),udsc_c(3),ducs_c(3)
     ...
     1     mv43(6,4,0:2)
      complex*16 virt(4,2),born(2,2),mbox21(-1:1,-1:1,0:2,2),
     1     mbox43(-1:1,-1:1,0:2,2)
...
c variables for powheg:
     ...
     C     l23squared,lb3squared      
      ...
      double complex dotcc, contract_Tjj
      external dotcc, contract_Tjj
...
C     always correctly tell us which side has the "extra" tag parton)
         ig = flst_borntags(7,flst_cur_iborn) 
...
C     so look for a 0 in position 1:2 or 4:6 
         ...
         if (any(flst_borntags(5:7,flst_cur_iborn) ==0)) ig = 0  
         if (ig .eq.1) then 
            lbox43 = .false.
         elseif (ig .eq.2) then 
            lbox21 = .false.
...
c     born-like amplitudes
                     born(1,(k+3)/4) = jh1(0,isig1)*j43h(0,isig3,(k+3)/4)-
     1                    jh1(1,isig1)*j43h(1,isig3,(k+3)/4)-
     2                    jh1(2,isig1)*j43h(2,isig3,(k+3)/4)-
     3                    jh1(3,isig1)*j43h(3,isig3,(k+3)/4)
                     born(2,(k+3)/4) = jh2(0,isig1)*j43h(0,isig3,(k+3)/4)-
     1                    jh2(1,isig1)*j43h(1,isig3,(k+3)/4)-
     2                    jh2(2,isig1)*j43h(2,isig3,(k+3)/4)-
     3                    jh2(3,isig1)*j43h(3,isig3,(k+3)/4)
c     
                     call boxline_vg(psi(1,isig1,1),psi(1,isig1,2),
     1                    p(0,1),p(0,2),isig1,
     2                    eps1(0,i),j43h(0,isig3,(k+3)/4),q,p21g,
     3                    born(1,(k+3)/4),virt(1,(k+3)/4))
c
                     mbox21(isig1,isig3,i,(k+3)/4) = virt(4,(k+3)/4)
                  enddo !isig1
               enddo !isig3
            enddo ! gluon pol (i)
         endif !lbox21
         ...
         if(lbox43) then
            ...
            do i=0,2               ! gluon polarizations
               ...
               do isig1 = -1,1,2    ! fermion 1 chirality
                  do isig3 = -1,1,2 ! fermion 3 chirality
c     born-like amplitudes
                     born(1,(k+3)/4) = jh1(0,isig3)*j21h(0,isig1,(k+3)/4)-
     1                 jh1(1,isig3)*j21h(1,isig1,(k+3)/4)-
     2                 jh1(2,isig3)*j21h(2,isig1,(k+3)/4)-
     3                 jh1(3,isig3)*j21h(3,isig1,(k+3)/4)
                     born(2,(k+3)/4) = jh2(0,isig3)*j21h(0,isig1,(k+3)/4)-
     1                 jh2(1,isig3)*j21h(1,isig1,(k+3)/4)-
     2                 jh2(2,isig3)*j21h(2,isig1,(k+3)/4)-
     3                 jh2(3,isig3)*j21h(3,isig1,(k+3)/4)

                     call boxline_vg(psi(1,isig3,3),psi(1,isig3,4),
     1                    p(0,3),p(0,4),isig3,
     2                    eps1(0,i),j21h(0,isig1,(k+3)/4),q,p43g,
     3                    born(1,(k+3)/4),virt(1,(k+3)/4))
                     mbox43(isig1,isig3,i,(k+3)/4) = virt(4,(k+3)/4)
...
c     born: 
     ...
     $                 e21j43(isig1,isig3,i,(k+3)/4) !born term 21 line
c     
                  mm43(k,isig,i) = fcpl(isig,k)*
     $                 j21e43(isig1,isig3,i,(k+3)/4) !born term 43 line
c     
c     boxes:
                  if (nlo.ne.0) then
c
                  if(lbox21) then !check that the correct factors are here
                     mv21(k,isig,i) = fcpl(isig,k)*
     1                    mbox21(isig1,isig3,i,(k+3)/4) +
     2                    colfac*cvirtH3j*mm21(k,isig,i)     !NOT NEEDED HERE, ADD ALL TOGETHER AT THE END
                  ...
                  if(lbox43) then
                     mv43(k,isig,i) = fcpl(isig,k)*
     1                    mbox43(isig1,isig3,i,(k+3)/4)
...
!      parameter (ca=3d0,cf=4d0/3d0,tr=0.5d0,nf=4d0)
      parameter (ca=3d0,cf=4d0/3d0,tr=0.5d0)
