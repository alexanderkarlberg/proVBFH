...
C     (based on comments above about colour structure of the amplitudes) 
      if (any(tags(1:2) == 0) .or. any(tags(5:8) == 0)) then 
...
!     ! Z
      if(nc_type) then
         facZ = fpials(2)*fpials(3)
      endif
!     ! W
      if(cc_type) then
         facW = fpials(2)*fpials(3)
      endif
...
!     12-x
     ...
     &                                    jgg(1,isig1,l1,l2,1,1),jx(1,isig3,2))
                        endif
!     21-x
                        if(tfac(3).ne.0d0) then
                           mat(isig1,isig3,l1,l2,3) =
     &                       contract_Tjj(vvhh_re(0,0,1,1),
     &                                    jgg(1,isig1,l2,l1,2,1),jx(1,isig3,2))
                        endif
                     endif
!     1-2
                     if(tfac(2).ne.0d0) then
                        mat(isig1,isig3,l1,l2,2) =
     &                    contract_Tjj(vvhh_re(0,0,3,1),
     &                                 jg(1,isig1,l1,1,1),jg(1,isig3,l2,2,2))
                     endif
                  endif
                  if ( .not.lgoff(2) ) then
                     if ( .not.lgoff(3) ) then
!     2-1
     ...
     &                       contract_Tjj(vvhh_re(0,0,4,1),
     &                                    jg(1,isig1,l2,2,1),jg(1,isig3,l1,1,2))
                        endif
                     endif
                     if ( .not.lgoff(3) ) then
!     x-12
     ...
     &                                    jx(1,isig1,1),jgg(1,isig3,l1,l2,1,2))
                        endif
!     x-21
                        if(tfac(6).ne.0d0) then
                           mat(isig1,isig3,l1,l2,6) =
     &                       contract_Tjj(vvhh_re(0,0,2,1),
     &                                    jx(1,isig1,1),jgg(1,isig3,l2,l1,2,2))
                        endif
                     endif
                  endif
               enddo
...
!!!   lower line
     ...
     &                             fac*facZ(5)
      ...
      if(cc_type) then
! Recontract with CC leptonic tensor - only left-handed
      mat=0
      isig1=-1
      isig3=-1
      do l1=-1,1,2
...
c------------------------------------------------------------------------
      ...
      double precision q12(0:3), q12g12(0:3), q12g1(0:3), q12g2(0:3)
      double precision q34(0:3), q34g12(0:3), q34g1(0:3), q34g2(0:3)

c momentum setup
      q12 = p(:,1)-p(:,5)
      q12g1 = q12-p(:,nlegreal-1)
      q12g2 = q12-p(:,nlegreal)
      q12g12 = q12g1-p(:,nlegreal)
      q34 = p(:,2)-p(:,6)
      q34g1 = q34-p(:,nlegreal-1)
      q34g2 = q34-p(:,nlegreal)
      q34g12 = q34g1-p(:,nlegreal)

c get leptonic tensors for Higgs pair production
      CALL SXXXXX(p(0,3),1,wp)
      CALL SXXXXX(p(0,4),1,wm)
      
      call ZZtoHH(q12g12,q34,p(0,3),vvhh_re(0,0,1,1))
      call ZZtoHH(q12,q34g12,p(0,3),vvhh_re(0,0,2,1))
      call ZZtoHH(q12g1,q34g2,p(0,3),vvhh_re(0,0,3,1))
      call ZZtoHH(q12g2,q34g1,p(0,3),vvhh_re(0,0,4,1))
      call WWtoHH(q12g12,q34,p(0,3),vvhh_re(0,0,1,2))
      call WWtoHH(q12,q34g12,p(0,3),vvhh_re(0,0,2,2))
      call WWtoHH(q12g1,q34g2,p(0,3),vvhh_re(0,0,3,2))
      call WWtoHH(q12g2,q34g1,p(0,3),vvhh_re(0,0,4,2))
